...
export default function Skills() {
  ...
  return (
    <section className="max-w-5xl mx-auto py-16 px-4 bg-background text-gray-200">
      <h2 className="text-3xl font-heading text-center mb-6 text-primary">Skills &amp; Tools</h2>
      ...
      >
        <AnimatePresence>
          {filtered.map(skill => (
            <motion.span
              key={skill.name}
              className="relative flex flex-col items-center justify-center bg-background rounded-full shadow-soft cursor-pointer min-w-[128px] min-h-[128px] w-32 h-32 group transition-transform hover:scale-110 border border-gray-700"
              variants={{
            ...
            >
              {skill.image ? (
                <span className="flex items-center justify-center w-44 h-44 bg-background rounded-full border border-primary">
              ...
              {skill.image && (
                <span className="absolute inset-0 flex items-center justify-center bg-primary bg-opacity-80 text-white text-xl font-bold rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                  {skill.name}
                </span>
              )}
            </motion.span>
